...
- **~15-30s typical latency per turn** — Each turn waits for the slowest of N parallel members plus serial Chairman synthesis. With the reference quick-model lineup, individual members usually return in 4-13s and the Chairman in 5-15s. Member responses are capped at 40s by `MEMBER_TIMEOUT_MS`; if one touches the cap, the turn lands in the 60-90s tail (uncommon). Fine for chat-paced workflows (Discord, async agents); poor for streaming UIs that expect <2s first token. *To trim latency: swap a slow member for a faster fast/flash-tier model, or reduce from 4 to 3 members.*
